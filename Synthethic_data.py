...
class MultiTable:
    ...
    def set_tables(self) -> None:
        ...
        The function populates self.tables
        """
    ...
    def set_metadata(self) -> None:
        """
        Initialize variables by reading CSV and JSON files into memory. 
        The function populates self.metadata.
        """
        # Paths to data, metadata, and relationship files
        location = f"datasets/{self.dataset_name}"
    ...
    def get_key_relationship(self, id_rel):
        """
        The function returns information about the relationship
        """
    ...
    def get_parents(self, table_name):
        """
        The function returns the list of parents
        """
        parents = []
        for relationship in self.metadata["relationships"]:
    ...
    def get_children(self, table_name):
        """
        The function returns the list of children
        """
        children = []
        for relationship in self.metadata["relationships"]:
    ...
    def get_numerical_int_columns(self, table_name):
        """
        The function returns the list of integer columns
        """
        numerical_columns = []
    ...
    def get_numerical_float_columns(self, table_name):
        """
        The function returns the list of float columns
        """
        numerical_columns = []
        for column_name,column_value in self.metadata["tables"][table_name]["columns"].items():
            if column_value["sdtype"] == "numerical":
    ...
    def get_numerical_columns(self, table_name):
        """
        The function returns the list of numerical columns
        """
        numerical_columns = []
        for column_name,column_value in self.metadata["tables"][table_name]["columns"].items():
            if column_value["sdtype"] == "numerical":
    ...
    def get_categorical_columns(self, table_name):
        """
        The function returns the list of categorical columns
        """
        categorical_columns = []
        for column_name,column_value in self.metadata["tables"][table_name]["columns"].items():
    ...
    def get_c_n_columns(self, table_name):
        """
        The function returns the list of numerical and categorical columns
        """
        c_n_columns = []
        for column_name,column_value in self.metadata["tables"][table_name]["columns"].items():
    ...
    def extend_parent(self, table_name, parent):
        ...
        categorical and numerical column in the parent table
        """
    ...
    def extend_data_table(self, table_name):
        """
        The functions extend the table by adding information about the parents and children
        """
...
class Synthethizer:
    ...
    def get_parent_columns(self, name_parent, id_relationship):
        """
        The function retrieves the categorical and numerical columns from the parent table, 
        then include the column that stores capacity. 
        This step is crucial because it ensures all relevant records can be properly linked.
        """
    ...
    def get_child_columns(self, name_parent, id_relationship):
        """
        The function obtains the list of columns that belongs to the child table
        The 'id_relationship' column was added to prevent two columns from having the same name.
        """
        # Obtain the list of columns (categorical + numerical) from parent
    ...
    def filter_relevant_columns(self, table_name, name_parent, id_relationship):
        ...
        However in the child table we stored these columns with id_relationship prefix
        """
    ...
    def convert_columns(self, table_name, name_parent, id_relationship):
        """
        This function converts the categorical and numerical columns 
        to a numerical representation, this allows us to realize calculations 
        """
        # First we filter that columns we care about
    ...
    def connect_related_tables(self, table_name, name_parent, id_relationship):
        """
        This function connects the related tables 
        """
    ...
    def connect(self):
        """
        For each table in the dataset, we generate synthetic data that includes valid foreign key 
        relationships to its parent tables. The resulting data is then saved to CSV files.
        """
        for table_name in self.dataset.tables.keys():
            ...
            self.synthethic_tables[table_name].to_csv(
                            f'datasets/{self.dataset.dataset_name}/ed_data/{table_name}.csv',
                            index=False
                        )

    def fit(self):
        """
        Generate synthetic data for each extended table, then connect each extended table 
        to its parent tables to establish valid foreign key relationships.
        """
...
if __name__ == "__main__":
    ...
    # Create a synthesizer instance with the prepared dataset
    synthethizer = Synthethizer(dataset=dataset)

    # Generate synthetic data for the extended tables and connect them 
    # to ensure valid foreign key relationships.
    synthethizer.fit()
